...
head(mod2$res)
# MSE:
sqrt(sum(mod2$res^2)/(dim(train.sub)[1] - 14)) # dim(train.sub)[1] - 14 = 621151 degree of freedom;14:非intercept
...
plot(mod2$fit, mod2$res, xlab = 'Fitted', ylab = 'residual')# residual
...
plot(exp(mod2_1$fit), train.sub$int_rate -exp(mod2_1$fit), xlab = 'Fitted', ylab = 'residual')# 由于mod2:residual logfit-logactual
...
par(mar = c(5.1,6,4.1,2.1))# mar:离四边的距离
...
plot(cvfit)# standanrd deviateion, model smple and mse small
...
# logisitic regression, loan no fully paid
